...
```bash
git reset --soft HEAD~5 # 到未合并的提交位置
git add # add 已跟踪的文件
gcmsg ""
gp -f
```
...
而 `git pull` 则是会获取所有远程索引并合并到本地分支中来。
...
`srcs = glob(["**/*.proto"])`：这行使用 glob 方法来选择当前目录及子目录下的所有 .proto 文件。
`**/*.proto` 表示递归查找所有 .proto 文件，并将它们作为源文件传递给该规则。

### deps

```plain
deps = [
    ":entity",  # 当前目录中的 entity 目标
    "entity",   # 当前目录中的 entity，但依赖于 Bazel 的查找逻辑
]
```

带冒号：明确当前目录中的目标，无路径歧义风险
不带冒号：优先解析为当前目录的目标，但存在潜在歧义
...
build --copt -Wno-deprecated-declarations # 抑制与源代码中使用弃用声明相关的警告
...
`visibility = ["//visibility:public"],`：授予对所有软件包的访问权限。
最佳实践：避免将 default_visibility 设为公开。
